...
export default function App() {
  ...
  // Load this account's saved scores after signing in.
  useEffect(() => {
    if (!session) {
      setScoreSummary(null);
      return;
    }
    fetchMyScores(session.token)
      .then(setScoreSummary)
      .catch(() => {
        clearSession();
        setSession(null);
      });
  ...
  // Persist the final score for signed-in players only; guests are never stored.
  useEffect(() => {
    if (gameEnded && session && !scoreSubmittedRef.current) {
      scoreSubmittedRef.current = true;
      submitScore(session.token, score)
        .then(setScoreSummary)
        .catch(() => {});
    }
  }, [gameEnded, session, score]);
  ...
  if (!session && !isGuest) {
    return <AuthScreen onAuthenticated={handleAuthenticated} onPlayAsGuest={handlePlayAsGuest} />;
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-md flex items-center justify-between mb-4 text-sm text-amber-800">
        <span>{session ? `Playing as ${session.username}` : 'Playing as Guest'}</span>
        <Button variant="ghost" size="sm" onClick={session ? handleSignOut : () => setIsGuest(false)}>
      ...
      <div className="text-center mb-8">
        ...
        {session && scoreSummary?.best != null && (
          <p className="text-amber-700 text-sm mt-2">
            Personal best: <span className="text-green-600">${scoreSummary.best}</span>
          </p>
        )}
      </div>
